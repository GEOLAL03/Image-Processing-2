...
function save_d_summary_image(statsTable, outputPath)
    ...
    for noiseIdx = 1:numel(noiseTypes)
        subplot(2, 1, noiseIdx);
        hold on;
        for methodIdx = 1:numel(methods)
            rows = strcmp(statsTable.NoiseType, noiseTypes{noiseIdx}) & ...
                strcmp(statsTable.Stage, 'Denoising') & ...
                strcmp(statsTable.Method, methods{methodIdx});
            if ~any(rows)
                continue;
            end
            [xValues, sortIdx] = sort(statsTable.ImageNumber(rows));
            yValues = statsTable.PSNR(rows);
            yValues = yValues(sortIdx);
            plot(xValues, yValues, '-o', 'LineWidth', 1.4, 'MarkerSize', 4, 'DisplayName', methods{methodIdx});
        end
        hold off;
        grid on;
        title(noiseTypes{noiseIdx});
        xlabel('Image number');
        ylabel('PSNR (dB)');
    ...
    try
        sgtitle('Part D - Denoising filters summary');
    catch
    end
